...
export const FilesView: React.FC = () => {
  ...
  const getFileIcon = (category: FileItem['category'], ext: string) => {
    if (category === 'code' || ['js', 'ts', 'jsx', 'tsx', 'py'].includes(ext)) {
      return <FileCode className="h-5 w-5 text-[#FF204E]" />;
    }
    if (category === 'data' || ['csv', 'json', 'xlsx'].includes(ext)) {
      return <FileSpreadsheet className="h-5 w-5 text-[#FF204E]" />;
    }
    if (category === 'image') {
      return <FileImage className="h-5 w-5 text-[#E50914]" />;
    }
    return <FileText className="h-5 w-5 text-[#E50914]" />;
  };
            ...
            <div>
              <div className="flex items-center justify-between gap-2 mb-2">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <h3 className="text-xs font-bold text-[#F8FAFC] truncate max-w-[160px] group-hover:text-[#FF204E] transition-colors">
                      {file.name}
                    </h3>
