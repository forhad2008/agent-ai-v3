...
export const ToolExecutionCard: React.FC<ToolExecutionCardProps> = ({ tool }) => {
  ...
  return (
    <div className="my-2 rounded-xl bg-[#0f0306] border border-[#E50914]/30 text-xs overflow-hidden">
      <div
        onClick={() => setExpanded(!expanded)}
        className="flex cursor-pointer items-center justify-between px-3.5 py-2.5 hover:bg-[#E50914]/15 transition-colors"
      >
        <div className="flex items-center gap-2">
          {getToolIcon()}
          <span className="font-mono text-[#F8FAFC] font-medium">{tool.toolName}</span>
          <span className="rounded-lg bg-[#080204] px-2 py-0.5 text-[10px] text-[#94A3B8] border border-[#E50914]/20">
            {tool.category}
          </span>
        </div>

        <div className="flex items-center gap-2">
          <span className="flex items-center gap-1 text-[11px] font-medium text-[#FF204E]">
      ...
      {expanded && (
        <div className="border-t border-[#E50914]/20 bg-[#080204] px-3.5 py-2.5 text-[#F8FAFC]">
