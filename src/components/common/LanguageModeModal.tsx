  ...
  return (
    ...
    >
      <div
        ...
        className="relative flex flex-col w-full max-w-2xl max-h-[90vh] rounded-3xl neumorph-card overflow-hidden text-[#F8FAFC] shadow-2xl animate-scaleUp"
      >
        {/* Modal Header */}
        <div className="flex items-start justify-between border-b border-[#E50914]/20 p-4 sm:p-6">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl neumorph-circle text-[#FF204E]">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="rounded-full neumorph-badge px-2.5 py-0.5 text-[10px] font-mono text-[#FF204E]">
                  30 Tech Nations
                </span>
              </div>
        ...
        <div className="flex items-center justify-between px-4 sm:px-6 py-2.5 neumorph-inset border-b border-[#E50914]/20 text-xs">
          <div className="flex items-center gap-2">
        ...
        <div className="p-4 sm:px-6 sm:py-3 border-b border-[#E50914]/20 space-y-3">
          {/* Search Box */}
          <div className="relative">
            ...
            <input
              ...
              className="w-full rounded-xl neumorph-inset pl-10 pr-4 py-2 text-xs sm:text-sm text-[#F8FAFC] placeholder:text-[#94A3B8]/60 focus:outline-none"
            />
            {search && (
              <button
                onClick={() => setSearch('')}
                className="absolute right-3.5 top-1/2 -translate-y-1/2 text-xs text-[#94A3B8] hover:text-[#F8FAFC] cursor-pointer"
              >
                Clear
              </button>
            )}
        ...
        {/* Language Grid */}
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 max-h-[50vh]">
              ...
              {filteredLanguages.map((lang) => {
                ...
                return (
                  <button
                    ...
                    className={`text-left flex items-center justify-between p-3 rounded-xl transition-all duration-200 group cursor-pointer ${
                      isActive
                        ? 'neumorph-btn-primary text-white'
                        : 'neumorph-card hover:border-[#FF204E]/40 text-[#94A3B8] hover:text-[#F8FAFC]'
                    }`}
                  >
                    <div className="flex items-center gap-2.5 min-w-0">
                      ...
                      <div className="min-w-0 flex items-center gap-1.5 flex-wrap">
                        ...
                        <span className="text-[10px] text-white/60 group-hover:text-white/80 truncate">
                          ({lang.name})
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center shrink-0 ml-2">
                      {isActive ? (
                        <span className="flex h-5 w-5 items-center justify-center rounded-full bg-white text-[#E50914] shadow-sm font-bold">
        ...
        {/* Modal Footer */}
        <div className="flex items-center justify-between p-4 sm:px-6 border-t border-[#E50914]/20 text-xs">
          <span className="text-[#94A3B8]">
            Selected: <strong className="text-[#F8FAFC]">{currentLanguage.country}</strong> ({currentLanguage.englishName})
          </span>
          <button
            onClick={() => setIsLanguageModalOpen(false)}
            className="rounded-xl neumorph-btn-secondary px-4 py-2 font-semibold text-[#FF204E] hover:text-white cursor-pointer"
          >
            {t.closeModal}
          </button>
        </div>
